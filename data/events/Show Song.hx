...
function create(){
    ...
    titleLine2 = new FlxSprite(566, titleText.y - 14).makeGraphic(Std.int(Math.max(titleText.width)), 5, FlxColor.BLACK);
    titleLine2.cameras = [camSong];
    titleLine2.screenCenter(FlxAxes.X);
    titleLine2.alpha = 0;

    titleLine1 = new FlxSprite(titleLine2.x - 5, titleLine2.y - 2).makeGraphic(Std.int(Math.max(titleText.width + 10)), 8, gbcolor);
    ...
    porterLine1 = new FlxSprite(porterLine2.x - 5, porterLine2.y - 2).makeGraphic(Std.int(Math.max(autorText.width + 10, autorText.width + 10)), 8, gbcolor);
